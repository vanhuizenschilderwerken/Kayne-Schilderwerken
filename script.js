...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const phone = "31612790499"; // Nederlands nummer in internationaal formaat, zonder +
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Probeer alle video's automatisch af te spelen
  videos.forEach((video) => {
    ...
    if (playPromise !== undefined) {
      playPromise.catch(() => {
        // Sommige browsers blokkeren autoplay tijdelijk
      });
    }

    // Pauzeer bij hover uit, speel af bij hover aan
...
document.addEventListener("DOMContentLoaded", () => {
  const ralFilters = document.querySelectorAll(".ral-filter");
  const ralSwatches = document.querySelectorAll(".ral-swatch");
  const ralPreviewImage = document.getElementById("ral-preview-image");

  if (!ralPreviewImage || !ralSwatches.length) return;

  function setActiveSwatch(targetSwatch) {
    ralSwatches.forEach((swatch) => swatch.classList.remove("active"));
    targetSwatch.classList.add("active");

    const newImage = targetSwatch.dataset.image;
    const newAlt = targetSwatch.dataset.alt || "";

    ralPreviewImage.src = newImage;
    ralPreviewImage.alt = newAlt;
  }

  function applyFilter(filter) {
    ralSwatches.forEach((swatch) => {
      const category = swatch.dataset.category;
      const shouldShow = filter === "all" || category === filter;

      swatch.classList.toggle("is-hidden", !shouldShow);
    });

    const activeVisibleSwatch = document.querySelector(".ral-swatch.active:not(.is-hidden)");
    if (activeVisibleSwatch) return;

    const firstVisibleSwatch = document.querySelector(".ral-swatch:not(.is-hidden)");
    if (firstVisibleSwatch) {
      setActiveSwatch(firstVisibleSwatch);
    }
  }
  ...
  ralFilters.forEach((filterButton) => {
    filterButton.addEventListener("click", () => {
      const filter = filterButton.dataset.filter;

      ralFilters.forEach((btn) => btn.classList.remove("active"));
